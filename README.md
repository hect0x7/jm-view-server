...
这次更新把文件浏览、管理操作和设置中心进一步整合，让大型本地漫画库更容易查找和维护：

- **Finder 分栏浏览**：最多同时展示四层目录，继续深入时自动向左推进；刷新后按当前路径恢复目录链，窄屏自动切换为单列导航。
- **操作入口可控**：分栏每行使用紧凑的 `⋯` 菜单，并可在首页或设置页统一隐藏；偏好会在多个浏览器标签页之间同步。
- **批量操作更顺手**：多选模式下点击列表行或网格卡片的普通区域即可选择，链接、按钮和管理操作仍保持优先。
- **文件管理更一致**：删除已浏览到的共享根外普通文件时保留盘符根、默认共享根和系统目录安全护栏；上传始终使用页面明确展示的目标目录。
- **设置页不再跳动**：阅读开关、主题、范围控件和侧栏操作均保持当前内容锚点，分类菜单与移动端动态视口不会把页面滚出 App Shell。

---

## 📖 v0.3.0 阅读器大升级

这次更新把原本以连续下拉为主的看本页面，升级成了一套完整的本地漫画阅读器：

| 阅读模式 | 适合场景 | 主要能力 |
| --- | --- | --- |
| **连续下拉** | 条漫、快速浏览 | 顺滑滚动、懒加载、阅读进度记忆 |
| **单页** | 传统漫画、键盘操作 | 左右点击翻页、方向键 / PageUp / PageDown / 空格翻页、适应窗口或自定义尺寸 |
| **双页** | 横屏桌面阅读 | 纵图自动成对、横图自动跨页、左右与上下无缝衔接、支持从左到右或从右到左 |

- **双页画面比例**：可在 `50%–100%` 之间调整画面占比，在满屏冲击力和舒适留白之间自由选择。
- **独立设置中心**：进入漫画前即可配置默认阅读模式、图片尺寸、阅读方向、主题、侧栏、消息昵称等浏览器偏好。
- **更顺手的工具栏**：桌面端可固定展开，也可恢复悬停自动收起；移动端使用轻量抽屉，避免遮挡画面。
- **阅读辅助完善**：缩略图总览、跳页、护眼、进度记忆、首次使用引导、快捷键表和长按旋转菜单全部内置。
- **滚动更容易抓取**：桌面滚动条更宽、更醒目；拖动滑块时显示实时阅读百分比，普通滚动时保持画面纯净。

*(电脑端：无缝双页模式、快捷工具栏与阅读进度)*
![](images/pc-jm-view-double-mode.webp)
...
### 1. 文件夹管理
- **快捷查找**：支持列表、网格和 Finder 分栏三种布局；列表与网格可通过文件夹封面快速识别内容，分栏适合连续深入多层目录。
- **书签直达**：一键将常看的目录收纳至侧边栏书签，下次打开无需一层层寻找。
- **日夜间模式**：内置深浅双色主题，自动记住你的偏好，夜间使用不刺眼。

**Finder 分栏浏览**：桌面端最多同时显示四层目录，继续深入时会将较早的列向左推进；移动端自动切换为更适合触控的单列导航。刷新页面后，会根据当前路径恢复最近四列目录，并重新选中对应的父级条目，方便从原位置继续浏览。

| 分栏能力 | 使用说明 |
| --- | --- |
| **紧凑操作菜单** | 每行只保留一个 `⋯` 入口，按文件类型显示进入、下载、看本、打包、重命名、移动、定位和删除等操作。 |
| **操作入口开关** | 视图切换旁的“操作”开关可隐藏每行入口，为文件名释放更多空间；默认开启并自动记忆，也可在设置页修改，多个标签页会同步更新。 |
| **键盘与触控** | 支持方向键、Enter 等键盘导航；移动端使用单列布局，返回和操作入口保留至少 `44px` 的触控区域。 |
| **快捷方式处理** | 点击快捷方式时进入它指向的目标位置，但重命名、移动和删除等管理操作始终作用于快捷方式本体。 |

*(电脑端：Finder 分栏视图)*
![](images/pc-index-column-mode.png)
...
- **三种阅读方式**：连续下拉、单页翻页和无缝双页随时切换，并可在设置中心选择默认模式。
- **操作顺手**：单页模式在电脑端支持键盘快捷键和左右点击画面翻页，连续下拉与双页模式保持自然滚动；手机端则专门优化了触屏排版和单手操作，还能随时切换暖色护眼模式。
